...
async function fbSavePost(post) {
  ...
  try {
    ...
    await _db.ref("posts/" + clean.id).set(clean);
    return { ok: true };
  } catch (e) {
...
async function fbLoadPosts() {
  ...
  try {
    const snap = await _db.ref("posts").orderByChild("updatedAt").once("value");
    const data = snap.val() || {};
...
function fbListenPosts(callback) {
  if (!_fbReady || !_db) { _readyCBs.push(() => fbListenPosts(callback)); return; }
  _db.ref("posts").orderByChild("updatedAt").on("value", snap => {
    const data = snap.val() || {};
    try { callback(Object.values(data).sort((a,b) => (b.updatedAt||0) - (a.updatedAt||0))); } catch(e){}
  }, err => console.error("Firebase listener:", err.message));
}
